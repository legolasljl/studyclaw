...
+ ### windows打开**studyclaw.exe**出现直接闪退
```yaml
  在文件路径栏输入**cmd**,然后再黑色命令窗口中输入```./studyclaw.exe```,
  然后查看报错内容截图并在[github](https://github.com/legolasljl/studyclaw/issues) 提交issue
```
...
```yaml
 ...
 nohup ./studyclaw > studyclaw.log 2>&1
...
```yaml
studyclaw进程会在运行的时候将pid输出到目录下的pid.pid文件，使用kil -9 命令即可退出后台进程
```
