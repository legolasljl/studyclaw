...
func initWechat() {
	...
	if len(list.KF_list) < 1 {
		err := wx.AddKFAccount("studyclaw@studyclaw", "studyclaw", utils.StrMd5("123"))
		if err != nil {
...
func sendMsg(id, message string) {
	...
	err := wx.PostText(id, message, "studyclaw@studyclaw")
	if err != nil {
...
func handleStartStudy(id string, msg string) {
	...
	for i, user := range users {
		...
		startTime := time.Now()
		lib.RunLearningWorkflow(core, user)

		state.Delete(user.Uid)
		score, _ := lib.GetUserScore(user.ToCookies())
		sendMsg(id, fmt.Sprintf("第%d个用户\n%v", i+1, lib.FormatLearningCompletionMessage(user.Nick, time.Since(startTime), score)))
	}
}
