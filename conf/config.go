...
type Config struct {
	...
	Study       struct {
		RecentDays            int  `json:"recent_days" yaml:"recent_days" mapstructure:"recent_days"`
		FallbackToAll         bool `json:"fallback_to_all" yaml:"fallback_to_all" mapstructure:"fallback_to_all"`
		NavigationRetryTimes  int  `json:"navigation_retry_times" yaml:"navigation_retry_times" mapstructure:"navigation_retry_times"`
		RetryBackoffSeconds   int  `json:"retry_backoff_seconds" yaml:"retry_backoff_seconds" mapstructure:"retry_backoff_seconds"`
		OperationIntervalSecs int  `json:"operation_interval_seconds" yaml:"operation_interval_seconds" mapstructure:"operation_interval_seconds"`
		ScoreRetryTimes       int  `json:"score_retry_times" yaml:"score_retry_times" mapstructure:"score_retry_times"`
		ArticleDurationSecs   int  `json:"article_duration_seconds" yaml:"article_duration_seconds" mapstructure:"article_duration_seconds"`
		ArticleDurationJitter int  `json:"article_duration_jitter_seconds" yaml:"article_duration_jitter_seconds" mapstructure:"article_duration_jitter_seconds"`
		VideoDurationSecs     int  `json:"video_duration_seconds" yaml:"video_duration_seconds" mapstructure:"video_duration_seconds"`
		VideoDurationJitter   int  `json:"video_duration_jitter_seconds" yaml:"video_duration_jitter_seconds" mapstructure:"video_duration_jitter_seconds"`
		AudioDurationSecs     int  `json:"audio_duration_seconds" yaml:"audio_duration_seconds" mapstructure:"audio_duration_seconds"`
		AudioDurationJitter   int  `json:"audio_duration_jitter_seconds" yaml:"audio_duration_jitter_seconds" mapstructure:"audio_duration_jitter_seconds"`
		// 登入失效時的最大重試次數
		AuthFailureThreshold int `json:"auth_failure_threshold" yaml:"auth_failure_threshold" mapstructure:"auth_failure_threshold"`
		// 連續失敗後的冷卻時間(分鐘)
		FailureCooldownMinutes int `json:"failure_cooldown_minutes" yaml:"failure_cooldown_minutes" mapstructure:"failure_cooldown_minutes"`
		// 滑塊驗證最大失敗次數
		MaxSliderFailures int `json:"max_slider_failures" yaml:"max_slider_failures" mapstructure:"max_slider_failures"`
		// 是否啟用快速失敗模式
		FastFailEnabled bool `json:"fast_fail_enabled" yaml:"fast_fail_enabled" mapstructure:"fast_fail_enabled"`
	} `json:"study" yaml:"study" mapstructure:"study"`
	Push struct {
		Ding struct {
	...
	// 每次定时任务执行的答题轮数，默认1轮
	DailyRounds int `json:"daily_rounds" yaml:"daily_rounds" mapstructure:"daily_rounds"`
...
func InitConfig(path string, restart func()) {
	...
	viper.SetDefault("scheme", "https://hamacd.github.io/nxxjump.github.io?")
	viper.SetDefault("model", 2)
	viper.SetDefault("study.recent_days", 180)
	viper.SetDefault("study.fallback_to_all", true)
	viper.SetDefault("study.navigation_retry_times", 4)
	viper.SetDefault("study.retry_backoff_seconds", 5)
	viper.SetDefault("study.operation_interval_seconds", 7)
	viper.SetDefault("study.score_retry_times", 3)
	viper.SetDefault("study.article_duration_seconds", 90)
	viper.SetDefault("study.article_duration_jitter_seconds", 20)
	viper.SetDefault("study.video_duration_seconds", 80)
	viper.SetDefault("study.video_duration_jitter_seconds", 15)
	viper.SetDefault("study.audio_duration_seconds", 80)
	viper.SetDefault("study.audio_duration_jitter_seconds", 15)
	viper.SetDefault("study.auth_failure_threshold", 5)
	viper.SetDefault("study.failure_cooldown_minutes", 5)
	viper.SetDefault("study.max_slider_failures", 5)
	viper.SetDefault("study.fast_fail_enabled", false)
